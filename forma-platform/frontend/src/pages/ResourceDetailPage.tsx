...
import {
  Star, Download, Eye, Heart, ShoppingCart, Tag, Shield,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <nav className="flex items-center gap-2 text-sm text-text-muted mb-6 animate-fade-in">
        ...
        <span>/</span>
        <Link to={`/catalog?typeId=${resource.type?.id}`} className="hover:text-primary-500 transition-colors">
          {resource.type?.name}
        </Link>
        <span>/</span>
        <span className="text-text-secondary truncate">{resource.name}</span>
      </nav>
        ...
        <div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className={`btn-outline px-4 py-4 ${isFav ? 'border-red-500 text-red-500' : ''}`}
