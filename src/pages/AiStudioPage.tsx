...
function ChatTab() {
  ...
  return (
    <div className="flex gap-4 relative" style={{ height: 'calc(100vh - 280px)' }}>
      {/* Mobile sidebar toggle */}
      <button
        onClick={() => setSidebarOpen(!sidebarOpen)}
        className="lg:hidden absolute top-2 left-2 z-20 p-2 rounded-lg bg-white shadow-md border border-surface-200 text-text-secondary hover:text-primary-500 transition-colors"
      >
      ...
      {/* Сайдбар сессий */}
      <div className={`
        w-64 shrink-0 card flex flex-col overflow-hidden
        max-lg:fixed max-lg:left-0 max-lg:top-0 max-lg:h-full max-lg:z-40 max-lg:rounded-none max-lg:shadow-xl
        max-lg:transition-transform max-lg:duration-300
        ${sidebarOpen ? 'max-lg:translate-x-0' : 'max-lg:-translate-x-full'}
      `}>
        <div className="p-3 border-b border-surface-200">
          <button onClick={() => { createNewChat(); setSidebarOpen(false) }} className="w-full btn-primary text-sm py-2 flex items-center justify-center gap-2">
