...
import { Users, Package, Flag, Wallet, CheckCircle, XCircle, AlertCircle, UserCheck, Trash2, Search } from 'lucide-react'
...
export default function AdminPage() {
  ...
  const [rejectTarget, setRejectTarget] = useState<{ id: number; type: 'resource' | 'author' } | null>(null)
  ...
  const { data: usersData, isLoading: uLoading } = useQuery({
    ...
    queryFn: () => adminApi.getUsers(usersPage, 20, debouncedSearch || undefined),
    enabled: tab === 'users',
  })

  const { data: pendingData, isLoading: pLoading } = useQuery({
    queryKey: ['admin-pending', resourcesPage],
    queryFn: () => adminApi.getPendingResources(resourcesPage, 20),
    enabled: tab === 'resources',
  })

  const { data: authorsData, isLoading: aLoading } = useQuery({
    queryKey: ['admin-authors', authorsPage],
    queryFn: () => adminApi.getPendingAuthors(authorsPage, 20),
    enabled: tab === 'authors',
  })

  const { data: complaintsData, isLoading: cLoading } = useQuery({
    queryKey: ['admin-complaints', complaintsPage],
    queryFn: () => adminApi.getComplaints(complaintsPage, 20),
    enabled: tab === 'complaints',
  })

  const { data: withdrawalsData, isLoading: wLoading } = useQuery({
    queryKey: ['admin-withdrawals', withdrawalsPage],
    queryFn: () => adminApi.getWithdrawals(withdrawalsPage, 20),
  ...
  const { items: pending, totalPages: pendingPages } = extract(pendingData)
  const { items: authors, totalPages: authorsPages } = extract(authorsData)
  const { items: users, totalPages: usersPages } = extract(usersData)
  const { items: complaints, totalPages: complaintsPages } = extract(complaintsData)
  const { items: withdrawals, totalPages: withdrawalsPages } = extract(withdrawalsData)

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <PageHead title="Администрирование" />
      <Breadcrumbs items={[{ label: 'Администрирование' }]} />
      <h1 className="text-3xl font-bold text-text-primary mb-8">Панель администратора</h1>

      {/* Tabs */}
      <div className="flex gap-1 bg-surface-200 rounded-xl p-1 mb-8 overflow-x-auto">
        {TABS.map((t) => (
          <button
            key={t.key}
        ...
        <div>
          ...
          {pLoading ? <div className="flex justify-center py-8"><Spinner /></div> :
            ...
            <>
                  ...
                  <div key={r.id} className="card p-5">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <div className="flex gap-2 shrink-0 flex-wrap">
                        ...
                        <button
                          onClick={() => setRejectTarget({ id: r.id, type: 'resource' })}
                          disabled={moderateMutation.isPending}
              ...
              <div className="mt-6">
                <Pagination page={resourcesPage} totalPages={pendingPages} onPageChange={setResourcesPage} />
              </div>
            </>
          )}
        </div>
        ...
        <div>
          ...
          {aLoading ? <div className="flex justify-center py-8"><Spinner /></div> :
            ...
            <>
                  ...
                  <div key={a.id} className="card p-5">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex gap-3 shrink-0">
                        ...
                        <button
                          onClick={() => setRejectTarget({ id: a.id, type: 'author' })}
                          disabled={authorVerifyMutation.isPending}
                          className="flex items-center gap-2 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-500 rounded-lg text-sm font-medium transition-colors"
                        >
                          <XCircle className="w-4 h-4" />
              ...
              <div className="mt-6">
                <Pagination page={authorsPage} totalPages={authorsPages} onPageChange={setAuthorsPage} />
              </div>
            </>
          )}
        </div>
      ...
      {tab === 'users' && (
        <div>
          ...
          {uLoading ? <div className="flex justify-center py-8"><Spinner /></div> : (
            ...
            <div className="mt-6">
              <Pagination page={usersPage} totalPages={usersPages} onPageChange={setUsersPage} />
            </div>
            </>
          )}
        </div>
        ...
        <div>
          ...
          {cLoading ? <div className="flex justify-center py-8"><Spinner /></div> :
            ...
            <>
              ...
              <div className="mt-6">
                <Pagination page={complaintsPage} totalPages={complaintsPages} onPageChange={setComplaintsPage} />
              </div>
            </>
          )}
        </div>
        ...
        <div>
          ...
          {wLoading ? <div className="flex justify-center py-8"><Spinner /></div> :
            ...
            <>
              ...
              <div className="mt-6">
                <Pagination page={withdrawalsPage} totalPages={withdrawalsPages} onPageChange={setWithdrawalsPage} />
              </div>
            </>
          )}
        </div>
      ...
      {rejectTarget && (
        <RejectModal
          title={rejectTarget.type === 'resource' ? 'Отклонение ресурса' : 'Отклонение заявки автора'}
          onConfirm={(reason) => {
            if (rejectTarget.type === 'resource') {
              moderateMutation.mutate({ id: rejectTarget.id, decision: 'REJECT', comment: reason })
            } else {
              authorVerifyMutation.mutate({ id: rejectTarget.id, decision: 'REJECT', comment: reason })
            }
